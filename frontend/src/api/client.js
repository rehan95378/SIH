...
export const register = (email, password) => request('/api/register', {
  method: 'POST',
  body: JSON.stringify({ email, password })
});

export const login = (email, password) => request('/api/login', {
  method: 'POST',
  body: JSON.stringify({ email, password })
});
...
export const uploadReport = (token, { title, dataType, content, file }) => {
  if (file) {
    ...
    return request('/api/ingest/file', authenticated(token, {
      method: 'POST',
      body: form
    }));
  }
...
export const getGraph = (token, sourceDocumentId) => request(
  sourceDocumentId
    ? `/api/graph?source_document_id=${encodeURIComponent(sourceDocumentId)}`
    : '/api/graph',
  authenticated(token)
);
export const getEntityRelationships = (token, entityId) => request(
  `/api/entities/${encodeURIComponent(entityId)}/relationships`,
  authenticated(token)
);
...
export const getAdminOverview = (token) => request(
  '/api/admin/overview',
  authenticated(token)
);
